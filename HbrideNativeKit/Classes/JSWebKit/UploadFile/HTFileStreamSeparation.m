...
#pragma mark - HTFileStreamSeparation
#define FileHashDefaultChunkSizeForReadingData 1024*8

@interface HTFileStreamSeparation()
...
@implementation HTFileStreamSeparation
...
-(void)setFileStatus:(HTUploadStatus)fileStatus
{
    _fileStatus = fileStatus;
    for (NSInteger num = 0; num<_streamFragments.count; num++) {
        HTStreamFragment *ft = _streamFragments[num];
        ...
        if (!ft.fragmentStatus) {
            _progressRate = (num+1.0)/_streamFragments.count;
            _uploadDateSize = HTStreamFragmentMaxSize * (num+1);
            break;
        }
    }
}
...
- (BOOL)getFileInfoAtPath:(NSString*)path {
    ...
    self.md5String = [HTFileStreamSeparation fileKeyMD5WithPath:path];
    ...
    self.fileStatus = HTUploadStatusWaiting;
...
- (void)cutFileForFragments {
    ...
    NSUInteger offset = HTStreamFragmentMaxSize;
    // 块数
    NSUInteger chunks = (_fileSize%offset==0)?(_fileSize/offset):(_fileSize/(offset) + 1);
    
    NSMutableArray<HTStreamFragment *> *fragments = [[NSMutableArray alloc] initWithCapacity:0];
    for (NSUInteger i = 0; i < chunks; i ++) {
        
        HTStreamFragment *fFragment = [[HTStreamFragment alloc] init];
...
@end
@implementation HTStreamFragment
