...
@interface HTTabBar ()
/** selctButton */
@property (weak , nonatomic) HTButton *selButton;
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        if ([HTTabBarConfig shared].haveBorder) {
            self.border.fillColor = [HTTabBarConfig shared].bordergColor.CGColor;
        }
        self.backgroundColor = [HTTabBarConfig shared].backgroundColor;
        
        [[HTTabBarConfig shared]addObserver:self forKeyPath:@"textColor" options:NSKeyValueObservingOptionNew context:nil];
        [[HTTabBarConfig shared]addObserver:self forKeyPath:@"selectedTextColor" options:NSKeyValueObservingOptionNew context:nil];
    }
    return self;
}
...
- (void)setItems:(NSArray<UITabBarItem *> *)items{
    ...
    for (int i=0; i<items.count; i++)
    {
        ...
        {
            self.centerBtn = [HTCenterButton buttonWithType:UIButtonTypeCustom];
        ...
        {
            btn = [HTButton buttonWithType:UIButtonTypeCustom];
            //Add Observer
            ...
            [self.btnArr addObject:(HTButton *)btn];
            [btn addTarget:self action:@selector(controlBtnClick:) forControlEvents:UIControlEventTouchUpInside];
        }
...
- (CAShapeLayer *)border{
    if (!_border) {
        ...
        border.path = [UIBezierPath bezierPathWithRect:
                       CGRectMake(0,0,self.bounds.size.width,[HTTabBarConfig shared].borderHeight)].CGPath;
...
- (void)layoutSubviews{
    ...
    NSInteger mid = [HTTabBarConfig shared].centerBtnIndex;
    ...
    _border.path = [UIBezierPath bezierPathWithRect:CGRectMake(0,0,
                                                               ...
                                                               [HTTabBarConfig shared].borderHeight)].CGPath;
}
 ...
 */
- (void)controlBtnClick:(HTButton *)button{
...
- (void)setSelectButtoIndex:(NSUInteger)index{
    if (self.centerBtn && index == self.centerBtn.tag) {
        self.selButton = (HTButton *)self.centerBtn;
    }else{
        for (HTButton *loop in self.btnArr){
            if (loop.tag == index){
                self.selButton = loop;
                break;
            }
 ...
 */
- (void)setSelButton:(HTButton *)selButton{
 ...
 *  Center button click
 */
- (void)centerBtnClick:(HTCenterButton *)button{
    //代理回调
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context
{
    if ([keyPath isEqualToString:@"badgeValue"] || [keyPath isEqualToString:@"badgeColor"]){
        HTButton *btn = (__bridge HTButton *)(context);
        btn.item = (UITabBarItem*)object;
    }
    else if ([object isEqual:[HTTabBarConfig shared]]){
...
- (void)dealloc{
    ...
    [[HTTabBarConfig shared]removeObserver:self forKeyPath:@"textColor" context:nil];
    [[HTTabBarConfig shared]removeObserver:self forKeyPath:@"selectedTextColor" context:nil];
}
