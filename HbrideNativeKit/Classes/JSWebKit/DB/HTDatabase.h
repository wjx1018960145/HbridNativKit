...
#import "HTResultSet.h"
#import "HTDatabasePool.h"
NS_ASSUME_NONNULL_BEGIN

#if ! __has_feature(objc_arc)
    #define HTDBAutorelease(__v) ([__v autorelease]);
    #define HTDBReturnAutoreleased FMDBAutorelease

    #define HTDBRetain(__v) ([__v retain]);
    #define HTDBReturnRetained FMDBRetain

    #define HTDBRelease(__v) ([__v release]);

    #define HTDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define HTDBAutorelease(__v)
    #define HTDBReturnAutoreleased(__v) (__v)

    #define HTDBRetain(__v)
    #define HTDBReturnRetained(__v) (__v)

    #define HTDBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details.
    #if OS_OBJECT_USE_OBJC
        #define HTDBDispatchQueueRelease(__v)
    #else
        #define HTDBDispatchQueueRelease(__v) (dispatch_release(__v));
    #endif
#endif
...
typedef int(^HTDBExecuteStatementsCallbackBlock)(NSDictionary *resultsDictionary);
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(__attribute__((noescape)) HTDBExecuteStatementsCallbackBlock _Nullable)block;
...
- (HTResultSet * _Nullable)executeQuery:(NSString*)sql, ...;
...
- (HTResultSet * _Nullable)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (HTResultSet * _Nullable)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
- (HTResultSet * _Nullable)executeQuery:(NSString *)sql values:(NSArray * _Nullable)values error:(NSError * _Nullable __autoreleasing *)error;
...
- (HTResultSet * _Nullable)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary * _Nullable)arguments;


// Documentation forthcoming.
- (HTResultSet * _Nullable)executeQuery:(NSString *)sql withVAList:(va_list)args;
...
@interface HTStatement : NSObject {
    void *_statement;
    NSString *_query;
    long _useCount;
    BOOL _inUse;
