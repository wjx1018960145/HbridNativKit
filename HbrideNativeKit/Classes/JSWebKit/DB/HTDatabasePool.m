...
#import "HTDatabase.h"
@interface HTDatabasePool () {
    dispatch_queue_t    _lockQueue;
    
    NSMutableArray      *_databaseInPool;
    NSMutableArray      *_databaseOutPool;
}
- (void)pushDatabaseBackInPool:(HTDatabase*)db;
- (HTDatabase*)db;

@end

@implementation HTDatabasePool
...
+ (instancetype)databasePoolWithURL:(NSURL *)url {
    return HTDBReturnAutoreleased([[self alloc] initWithPath:url.path]);
}

+ (instancetype)databasePoolWithPath:(NSString *)aPath flags:(int)openFlags {
    return HTDBReturnAutoreleased([[self alloc] initWithPath:aPath flags:openFlags]);
}

+ (instancetype)databasePoolWithURL:(NSURL *)url flags:(int)openFlags {
    return HTDBReturnAutoreleased([[self alloc] initWithPath:url.path flags:openFlags]);
}
...
- (instancetype)initWithPath:(NSString*)aPath flags:(int)openFlags vfs:(NSString *)vfsName {
    ...
    if (self != nil) {
        ...
        _databaseInPool     = HTDBReturnRetained([NSMutableArray array]);
        _databaseOutPool    = HTDBReturnRetained([NSMutableArray array]);
...
- (void)dealloc {
    ...
    _delegate = 0x00;
    HTDBRelease(_path);
    HTDBRelease(_databaseInPool);
    HTDBRelease(_databaseOutPool);
    HTDBRelease(_vfsName);
    
    if (_lockQueue) {
        HTDBDispatchQueueRelease(_lockQueue);
        _lockQueue = 0x00;
    }
