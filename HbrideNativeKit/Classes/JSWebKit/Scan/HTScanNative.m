...
#import "HTScanNative.h"
@interface HTScanNative()<AVCaptureMetadataOutputObjectsDelegate>
{
...
//扫码结果
@property (nonatomic, strong) NSMutableArray<HTScanResult*> *arrayResult;
 ...
 *  扫码结果返回
 */
@property(nonatomic,copy)void (^blockScanResult)(NSArray<HTScanResult*> *array);


@end
@implementation HTScanNative
...
- (instancetype)initWithPreView:(UIView*)preView ObjectType:(NSArray*)objType cropRect:(CGRect)cropRect success:(void(^)(NSArray<HTScanResult*> *array))block
{
    if (self = [super init]) {
...
- (instancetype)initWithPreView:(UIView*)preView ObjectType:(NSArray*)objType success:(void(^)(NSArray<HTScanResult*> *array))block
{
    if (self = [super init]) {
...
- (void)initParaWithPreView:(UIView*)videoPreView ObjectType:(NSArray*)objType cropRect:(CGRect)cropRect success:(void(^)(NSArray<HTScanResult*> *array))block
{
    ...
    [[self stillImageOutput] captureStillImageAsynchronouslyFromConnection:stillImageConnection
                                                         completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error)
     {
         ...
         if (imageDataSampleBuffer)
         {
             ...
             for (HTScanResult* result in _arrayResult) {
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection
{
    ...
    for(AVMetadataObject *current in metadataObjects)
    {
        if ([current isKindOfClass:[AVMetadataMachineReadableCodeObject class]] )
        {
            ...
            {
                HTScanResult *result = [HTScanResult new];
...
#pragma mark --识别条码图片
+ (void)recognizeImage:(UIImage*)image success:(void(^)(NSArray<HTScanResult*> *array))block;
{
    if ([[[UIDevice currentDevice]systemVersion]floatValue] < 8.0 )
    {
        if (block) {
            HTScanResult *result = [[HTScanResult alloc]init];
    ...
    NSMutableArray<HTScanResult*> *mutableArray = [[NSMutableArray alloc]initWithCapacity:1];
    for (int index = 0; index < [features count]; index ++)
    {
        ...
        HTScanResult *item = [[HTScanResult alloc]init];
