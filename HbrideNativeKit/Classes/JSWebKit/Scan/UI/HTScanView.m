...
@interface HTScanView()

//扫码区域各种参数
@property (nonatomic, strong,nullable) HTScanViewStyle* viewStyle;
...
//线条扫码动画封装
@property (nonatomic,strong,nullable)HTScanLineAnimation *scanLineAnimation;
//网格扫码动画封装
@property (nonatomic,strong,nullable)HTScanNetAnimation *scanNetAnimation;
...
- (void)startScanAnimation
{
    ...
    {
        case HTScanViewAnimationStyle_LineMove:
        {
            //线动画
            if (!_scanLineAnimation)
                self.scanLineAnimation = [[HTScanLineAnimation alloc]init];
            [_scanLineAnimation startAnimatingWithRect:_scanRetangleRect
                                                InView:self
                                                 Image:_viewStyle.animationImage];
        }
            break;
        case HTScanViewAnimationStyle_NetGrid:
        {
            //网格动画
            if (!_scanNetAnimation)
                self.scanNetAnimation = [[HTScanNetAnimation alloc]init];
            [_scanNetAnimation startAnimatingWithRect:_scanRetangleRect
                                               InView:self
                                                Image:_viewStyle.animationImage];
        }
            break;
        case HTScanViewAnimationStyle_LineStill:
        {
            if (!_scanLineStill) {
...
- (void)drawScanRect
{
  ...
  //画矩形框4格外围相框角
    ...
    {
        case HTScanViewPhotoframeAngleStyle_Outer:
        {
            diffAngle = linewidthAngle/3;//框外面4个角，与框紧密联系在一起
        }
            break;
        case HTScanViewPhotoframeAngleStyle_On:
        {
            diffAngle = 0;
        }
            break;
        case HTScanViewPhotoframeAngleStyle_Inner:
        {
